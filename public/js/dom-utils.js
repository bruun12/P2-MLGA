...
export function renderButtonElem({id, className, text, parent = document.body}={}) {
...
export function formatDates(array, dateKey, outputKey) {
    array.forEach(item => {
        // Convert the ISO UTC format to a JS Date object
        const date = new Date(item[dateKey]);

        // Extract and format each component of the date with leading zeros as needed
        const day = String(date.getUTCDate()).padStart(2, '0'); // Gets the day of the month, ensuring it's always 2 digits ("04" instead of "4").
        const month = String(date.getUTCMonth() + 1).padStart(2, '0'); // Gets the month (add 1 since JavaScript months are 0-based).
        const year = String(date.getUTCFullYear()).slice(-2); // Takes the year and changes it to the last 2 digits of the year (2028 → "28").
        const hours = String(date.getUTCHours()).padStart(2, '0'); // Hour in 24-hour format (00–23).
        const minutes = String(date.getUTCMinutes()).padStart(2, '0'); // Minutes in 24-hours format (00–59).
